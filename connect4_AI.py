...
def count_window(window, piece):
    score = 0
    oppon_piece = AI_PIECE if piece == PLAYER_PIECE else PLAYER_PIECE
...
def is_terminal(board):
    return is_win(board, PLAYER_PIECE) or is_win(board, AI_PIECE) or len(get_valid_columns(board)) == 0
...
def minmax(board, depth, max_player):
    ...
    if terminal or depth == 0:
        if is_win(board, PLAYER_PIECE):
            return (None, -10000)
        elif is_win(board, AI_PIECE):
            return (None, 10000)
        elif len(valid_columns) == 0:
            return (None, 0)
        else:
            return(None, heuristic_score(board,AI_PIECE))
    if max_player:
        score = -10000
        best_column = random.choice(valid_columns)
        for col in valid_columns:
            row = get_next_open_row(board, col)
            temp_board = board.copy()
            drop_piece(temp_board, col, row, AI_PIECE)
            _, new_score = minmax(temp_board, depth - 1, False)
            if new_score > score:
                score = new_score
                best_column = col
        ...
        for col in valid_columns:
            row = get_next_open_row(board, col)
            temp_board = board.copy()
            drop_piece(temp_board,col,row, PLAYER_PIECE)
            _, new_score = minmax(temp_board, depth - 1, True)
            if new_score < score:
                score = new_score
                best_column = col
        return best_column, score
...
while not game_over:
    ...
    if turn == AI and not game_over:
        pygame.time.wait(1250) #wait 1.5 sec before moving
